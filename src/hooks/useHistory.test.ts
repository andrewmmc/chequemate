  ...
  describe("addToHistory", () => {
    it("adds entry to history", () => {
      ...
      act(() => {
        result.current.addToHistory(
          100,
          "港幣壹佰元正",
          "Hong Kong Dollars One Hundred Only",
        );
      });
      ...
      expect(result.current.history[0].english).toBe(
        "Hong Kong Dollars One Hundred Only",
      );
    });
    ...
    it("prepends new entry to beginning", () => {
      ...
      act(() => {
        result.current.addToHistory(
          200,
          "港幣貳佰元正",
          "Hong Kong Dollars Two Hundred Only",
        );
      });
    ...
    it("persists to localStorage", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem("cheque-converter-history") || "[]",
      );
