...
interface ChequePreviewHKDProps {
  chinese: string;
  english: string;
}

export function ChequePreviewHKD({ chinese, english }: ChequePreviewHKDProps) {
    ...
    <div className="overflow-hidden rounded-[14px] border border-cm-border bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03),0_4px_16px_rgba(0,0,0,0.04)]">
      ...
      >
        <div className="flex items-center justify-between mb-3">
          ...
          <CopyButton value={chinese ? `港幣${chinese}` : ''} />
        </div>
        ...
        <div className="rounded-lg px-4 py-3 bg-white border border-cm-border">
          ...
          >
            <span className="text-ink-muted mr-1">港幣</span>
            {displayChinese}
          </p>
        </div>
      </section>
      ...
      <section className="px-5 pt-4 pb-5" aria-label={t('chequePreview.amountInEnglish')}>
        <div className="flex items-center justify-between mb-3">
          ...
          <CopyButton value={english ? `HKD ${english}` : ''} />
        </div>
        ...
        <div className="rounded-lg px-4 py-3 bg-white border border-cm-border">
          ...
          >
            <span className="text-ink-muted mr-1.5">HKD</span>
            {displayEnglish}
          </p>
        </div>
      </section>
